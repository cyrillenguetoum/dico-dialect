...
class Writer
{
     ...
     * File system adapter.
     *
     * @var \Search\Storage\Directory\DirectoryInterface
     */
     ...
     *
     * @var \Search\Index\SegmentWriter\DocumentWriter
     */
    ...
    public static function createIndex(Directory\DirectoryInterface $directory, $generation, $nameCount)
    {
        if ($generation == 0) {
            ...
            $segmentsFile = $directory->createFile(Index::getSegmentFileName($generation));
     ...
     * @param \Search\Storage\File\FileInterface $cleanUpLock
     */
    ...
    public function addDocument(Document $document)
    {
        if ($this->_currentSegment === null) {
            $this->_currentSegment = new SegmentWriter\DocumentWriter(
                $this->_directory, $this->_newSegmentName()
            );
        }
    ...
    {
        if (LockManager::obtainOptimizationLock($this->_directory) === false) {
            return;
        }

        if (!$this->_hasAnythingToMerge()) {
            LockManager::releaseOptimizationLock($this->_directory);
            return;
        }
    ...
    private function _updateSegments()
    {
        ...
        $generation = Index::getActualGeneration($this->_directory);
        $segmentsFile   = $this->_directory->getFileObject(Index::getSegmentFileName($generation), false);
        $newSegmentFile = $this->_directory->createFile(Index::getSegmentFileName(++$generation), false);
        ...
        try {
            // Write format marker
            if ($this->_targetFormatVersion == Index::FORMAT_2_1) {
                $newSegmentFile->writeInt((int)0xFFFFFFFD);
            } elseif ($this->_targetFormatVersion == Index::FORMAT_2_3) {
            ...
            if ($format == (int)0xFFFFFFFF) {
                $srcFormat = Index::FORMAT_PRE_2_1;
            } elseif ($format == (int)0xFFFFFFFD) {
                $srcFormat = Index::FORMAT_2_1;
            } elseif ($format == (int)0xFFFFFFFC) {
                $srcFormat = Index::FORMAT_2_3;
            } else {
            ...
            for ($count = 0; $count < $segmentsCount; $count++) {
                ...
                if ($srcFormat == Index::FORMAT_PRE_2_1) {
                ...
                if (!in_array($segName, $this->_segmentsToDelete)) {
                    ...
                    if ($this->_targetFormatVersion == Index::FORMAT_2_3) {
        ...
        if (LockManager::escalateReadLock($this->_directory)) {
            /**
             * Clean-up directory
             */
            $filesToDelete = array();
            ...
            foreach ($this->_directory->fileList() as $file) {
                if ($file == 'deletable') {
                    ...
                    if ($file != Index::getSegmentFileName($generation)) {
                        $filesToDelete[] = $file;
            ...
            LockManager::deEscalateReadLock($this->_directory);
        } else {
